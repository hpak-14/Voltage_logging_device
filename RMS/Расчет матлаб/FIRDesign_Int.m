...
w0 = kaiser(N,10.8); w0 = round(w0/max(w0)*65535);
w1 = chebwin(N,80); w1 = round(w1/max(w1)*65535);
...
file = fopen('KaiserWin1_Int.h','w');
fprintf(file,'const float kaiser_win_coeff = %1.15e;\n',1/sum(w0));
fprintf(file,'const unsigned short kaiser_win[] = { \n');
fprintf(file,'%d, \n',w0(1:end));
fprintf(file,'}; \n');
...
file = fopen('ChebyWin1_Int.h','w');
fprintf(file,'const float cheby_win_coeff = %1.15e;\n',1/sum(w1));
fprintf(file,'const unsigned short cheby_win[] = { \n');
fprintf(file,'%d, \n',w1(1:end));
fprintf(file,'}; \n');
